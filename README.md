...
Under the hood, Choicelab uses [Tauri](https://tauri.app), a framework for building cross-platform apps. It's sort of like Electron, in that the UI is built with HTML, CSS, and JavaScript — but the generated apps are much faster and _much_ smaller. While the app is mostly frontend, the backend is written in Rust (like Tauri) for handling basic setup, file management, and windowing — and a tiny bit of Swift to polyfill functionality Tauri doesn't have yet.
